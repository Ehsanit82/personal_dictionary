...
class WordBloc extends Bloc<WordEvent, WordState> {
  ...
  WordBloc(this._localDataSource, {required this.mode}) : super(WordState.initial(mode)) {
    ...
    on<AddWordEvent>((event, emit) async {
      ...
      _localDataSource.wordBox(box).add(wordData);
      emit(state.copyWith(wordList: state.wordList..add(wordData)));
    });
    on<FetchWordsEvent>((event, emit) {
      var box = getStringLanguageMode(state.mode);
     Box<Word> wordBox = _localDataSource.wordBox(box);
